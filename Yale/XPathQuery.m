      ...
      /*if ([[resultForNode objectForKey:@"nodeName"] isEqual:@"text"] && parentResult)
        ...
        if ([[resultForNode objectForKey:@"nodeName"] isEqual:@"text"] && parentResult)
        {
            currentNodeContent = [currentNodeContent
                                  stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
            NSString *parentNodeContent = [parentResult objectForKey:@"nodeContent"];
            if (parentNodeContent) {
                currentNodeContent = [parentNodeContent stringByAppendingString:currentNodeContent];
            }
...
NSArray *PerformXPathQuery(xmlDocPtr doc, NSString *query)
{
  ...
  xmlXPathRegisterNs(xpathCtx, (const unsigned char *)[@"yweather" cStringUsingEncoding:NSUTF8StringEncoding], (const unsigned char *)[@"http://xml.weather.yahoo.com/ns/rss/1.0" cStringUsingEncoding:NSUTF8StringEncoding]);
